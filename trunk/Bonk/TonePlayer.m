...
static const int TONESamplesPerBuffer = 44100 / 20;	// enough samples for 1/20th of a second  (the bigger this is, the longer the lag)
...
-(id)initWithFrequency: (float)f attack: (float)a decay: (float)d sustain: (float)s release: (float)r lifespan: (float)l
{
	[super init];
	frequency = f;
...
+(id)toneWithFrequency: (float)f attack: (float)a decay: (float)d sustain: (float)s release: (float)r lifespan: (float)l
{
	return [[[Tone alloc] initWithFrequency: f attack: a decay: d sustain: s release: r lifespan: l] autorelease];
}
...
static void AQBufferCallback( void *in, AudioQueueRef inQ, AudioQueueBufferRef outQB )
{
	...
	const float fullVolume = 32767;
	const float amplitude = fullVolume * player->volume;
	...
	for( currentSample=0; currentSample<TONESamplesPerBuffer; currentSample++ ) {
		...
		if( numberOfTones ) {
			for( i=0; i<numberOfTones; i++ ) {
				float s = [[playingTones objectAtIndex: i] nextSample] * (1.0f / numberOfTones);
				sampleValue += s;
			}
		}
